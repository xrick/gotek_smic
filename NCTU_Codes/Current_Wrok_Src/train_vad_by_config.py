...
def trainProcessEntry(h1_layer_num=512, h2_layer_num=32, batch_size = 40, learning_rate=0.008, \
    ...
    with tf.Session() as sess:
        ...
        spio.savemat("../weight/8+2band(25ms)/weight_{}x{}_{}_{}.mat".format( h1_layer_num, h2_layer_num, CurrentDateString,\
                                                                                 str(time.time()).replace(".","")),
                     {'w1': weight1, 'w2': weight2, 'w3': weight3, 'b1': bias1, 'b2': bias2, 'b3': bias3})

        saver.save(sess, "../ExpResults/model_{}x{}_lr_{}_{}".format(h1_layer_num, h2_layer_num, \
                                                                              current_lr_str, CurrentDateString, str(time.time()).replace(".","")))
...
time_log_file = "../time_log/timelog_{}_{}.log".format(CurrentDateString,str(time.time()).replace(".",""))
