...
class AudioReader:
    ...
    """
    def __init__(self, audio_dir, model_settings, background_noise, silence_label, unknown_label,
                 classes,augmentation_ops = [], augmentation_percentage = 0,
                 validation_percentage=10, testing_percentage = 10, unknown_percentage = 10,
                 silence_percentage = 10, testing_list = {}, validation_list= {},
                 fingerprint_type='mfcc' , mode='test', train=True):
    ...
    def augment_data(self, set_index):
        ...
        for label in train_data:
            ...
            train_data[label] = train_data[label][:int(len(train_data[label]) * self.augmentation_percentage / 100)]
            ...
            for file in train_data[label]:
                ...
                if not (isinstance(audio, str)):
                    # replace librosa with scipy.io.wavfile because librosa write out float number
                    audio = audio * 32768
                    # librosa.output.write_wav(aug_wav_path, audio.astype(np.float), self.model_settings['sample_rate'])
                    wavio.write(aug_wav_path,16000,audio.astype(np.int16))
    ...
    def get_data(self, how_many, offset, background_frequency, background_volume_range,
        ...
        print("The labels : {}".format(labels))
    ...
    def featurize_samples(self, background_frequency, background_volume_range,
        ...
        if features_2d:
            data =  np.zeros((sample_count,int(self.model_settings['fingerprint_size']/ self.model_settings['dct_coefficient_count']),self.model_settings['dct_coefficient_count']))
        else:
        ...
        for i in range(offset, offset + sample_count):
            ...
            if use_background:
                ...
                background_offset = np.random.randint(0, len(background_samples) - self.model_settings['desired_samples'])
