...
learning_rate = 0.01
batch_size = 1280
n_band = 13*40 # mfcc dim * context size #40*41
n_hidden1 = 128
n_hidden2 = 128
n_hidden3 = 128
n_classes = 3
...
def App_Run():
    ...
    w, b = get_init_weights()
    x = tf.placeholder(tf.float32, [None, n_band])
    y = tf.placeholder(tf.int8, [None, n_classes])
    x1 = tf.nn.relu(tf.add(tf.matmul(x,w['hidden1']),b['hidden1']))
    x2 = tf.nn.relu(tf.add(tf.matmul(x1,w['hidden2']),b['hidden2']))
    x3 = tf.nn.relu(tf.add(tf.matmul(x2,w['hidden3']),b['hidden3']))
    pred = tf.add(tf.matmul(x3,w['output']),b['output'])
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred, labels=y))
    # cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred, labels=y))+1e-8
